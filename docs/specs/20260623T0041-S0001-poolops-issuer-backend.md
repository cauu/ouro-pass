...
- [x] p11-2 产品/协议命名全量统一（破坏性，D12）：`POOLOPS_*`→`OUROPASS_*` env、`/.well-known/poolops/*`→`/.well-known/ouropass/*`、`iss=poolops:`→`ouropass:`、kid 前缀 `pao-issuer-`→`op-issuer-`、产品名 PoolOps→Ouro Pass；docs/v1.0 设计文档同步。代码 + 文档全绿。
...
- [x] p12-11 **P2 延后**（自 p12-9 拆出，D19）：destructive admin 操作的 step-up 策略统一——member-revoke / client-register / subscription-cancel 是否要求 owner step-up 是 API 契约 + 安全策略变更，需与（尚未实现的）admin UI 协同设计后再落地；当前仅 key-rotate 强制 step-up。
...
- 2026-06-23 p12-11 completed（应用户要求解除 D19 延后）：`adminRevokeMember` 与 `adminRegisterClient` 加 step-up——解析 body `{owner_vkey, step_up_nonce, step_up_signature}`、过 `requireStepUp` 才执行（revoke 在动作前、register 在请求形状校验后/落库前，保留坏枚举→400）。step-up 校验 session admin 自身 owner key（operator 亦可重签，属再认证非提权）。subscription-cancel 暂未纳入（爆炸半径低于 revoke/register）。
