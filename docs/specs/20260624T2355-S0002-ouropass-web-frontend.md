...
- [x] p6-5 **（多智能体评审"现在就修"项，用户批准）** 三项高确定性修复：① `server/Makefile` `dev-seed-client` 的 `INSERT` 仍引用已删列 `party`/`allowed_scopes`/`pkce_required`，迁移 0007/0008 后 `make dev-seed-client` 必失败——改为只插现存列，并把种子改为 `public` 客户端（旧的 confidential+NULL secret 本就无法完成兑换）、用 openssl 从固定 verifier 算 S256 challenge 打进授权 URL、附带 verifier（PKCE 现强制；已验证 openssl 与 Go `pkceS256` 输出一致）。② 补 confidential「有 secret、无 `code_verifier`→`ErrInvalidGrant`」负向测试（守 p6-3 闭合的缺口）。③ 修两处误导注释：`handlers_oauth.go:20` 的 `code_challenge?`（→必填）、`keys_test.go:127` 的 `(p5-1)`（→p5-2）。
- [x] p6-4 **（reveal client_id + regenerate secret，用户要求）** 支持查看 client_id、找回/重置 secret。澄清+决策：client_id 是公开标识符→列表加"Copy ID"（一键复制）；client_secret 单向哈希存储、明文不可还原→**不做"reveal 旧 secret"**（会逼降级为可逆存储），改为**Regenerate**（用户选 A，对齐 GitHub/Stripe）。后端 `POST /api/admin/oauth-clients/{client_id}/secret`（owner+step-up）：Get→仅 confidential（404/409 守卫）→新 `RandomToken(32)`、更新 hash、审计 `oauth_client.secret_regenerate`、回新明文一次。前端 `regenerateClientSecret()` + 客户端表 Actions 列：confidential 行"Regenerate secret"（step-up→一次性展示新 secret 面板，复制按钮），公开 client_id 加 Copy。
- [x] p6-3 **（强制 PKCE，删 `pkce_required`，用户定）** PKCE 对所有客户端强制（OAuth 2.1），删勾选框。① authorize 无条件要求 `code_challenge`（`oauth.go`，原"public 或 PKCERequired 才要"改为恒要、且 code 恒绑 challenge）；② **修补 confidential 兑换缺口**——`authenticateClient` 原对 confidential 只验 secret/跳过 PKCE，改为**先验 PKCE（所有客户端）再验 secret（confidential 额外）**，否则强制存的 challenge 永不校验；③ 删 `PKCERequired` 字段（domain/repo/handler/迁移 0008 DROP COLUMN）+ `boolToInt` 辅助；④ 前端去表单勾选框、`OAuthClient`/`ClientRegister`/`ClientForm` 去字段，注明"PKCE is required for all clients"。注：refresh_token 走独立 secret/device 校验、不涉 PKCE。
...
- 2026-06-25 p6-3 完成（强制 PKCE，用户"强制 pkce 把 删掉那个勾选框"，先讲清 PKCE S256 原理后落实）：排查发现一个**真缺口**——`authenticateClient` 原是 either/or（confidential 只验 secret 并 `return`，跳过 PKCE）；若只把 authorize 改成恒要 challenge，confidential 存了 challenge 却永不校验。故同时改两处：authorize 恒要 `code_challenge`（`oauth.go`，code 恒绑 challenge），`authenticateClient` 改为**对所有客户端先验 PKCE S256、confidential 再叠加 secret**。删 `PKCERequired` 端到端（domain/repo/handler/前端）+ 迁移 `0008_drop_client_pkce_required.sql`(sqlite+pg) + 删 `boolToInt`。测试：oauth 包加 `testPKCEVerifier/testPKCEChallenge` 共享辅助，`authorizeAs` 恒带 challenge，所有 confidential **authorization_code** 兑换补 `CodeVerifier`（token/introspect/refresh/reused-code 各测）；httpapi 两个 authorize 测试 body 补 `code_challenge`；e2e 的 web(confidential) 流程补 challenge+verifier、含两条 expect not_eligible 的 authorize 也需带 challenge 才能越过 PKCE 到达资格判定；删各测试 `PKCERequired` 字面量/断言。refresh_token 路径不涉 PKCE（独立 secret/device 校验），未改。

- 2026-06-25 p6-4 完成（reveal id + regenerate secret，用户"需要支持 reveal clientId 和 secret"）：先厘清——client_id 非机密（公开标识），secret 单向哈希存储、明文不可还原。用户选 A（regenerate，不改可逆存储）。后端：路由 `POST /oauth-clients/{client_id}/secret`（owner）+ `adminRegenerateClientSecret`（step-up→`Get`，`ErrNotFound`→404、非 confidential→409、否则新 `RandomToken(32)`+`HashToken` 经 `Get`+`Upsert` 落库，审计 `oauth_client.secret_regenerate`，回 `client_secret` 一次）。复用现有 repo（无新方法）。前端：`admin.ts` 加 `regenerateClientSecret`；`ClientsPage` 加 `CopyButton`（复制 client_id，toast）+ `RegenerateSecretAction`（StepUpDialog→`onConfirm` 拿新 secret→受控 `Dialog` 一次性展示+复制），表加 Actions 列（仅 confidential 显示 Regenerate）。测试：`TestAdminOwner_RegisterClientAndRotateKey` 扩展——无 step-up→401/403、有 step-up→新 secret≠原 secret、未知 client→404。

- 2026-06-26 p6-5 完成（评审"现在就修"项，用户"把值得现在就修的都修复"）：跑了 multi-agent-review（Claude+Cursor，Codex 未恢复跳过），产出 `code_review/S0002-keys-oauth-hardening/`。处置=只修高确定性低成本三项：① Makefile dev-seed（Cursor 抓到、我漏掉的真实破坏：INSERT 引用已删列→迁移后失败）；② confidential 无 verifier 负向测试（守 p6-3）；③ 两处误导注释。验证 openssl challenge 与 Go `pkceS256` 逐字节一致。其余评审发现按成本/收益判 defer/wontfix，见 §7 变更记录。
...
- 2026-06-25 p6-3 | stack: go | command: `go build ./...` + `go vet ./...` + `go test ./...` + `go vet -tags=integration ./internal/inttest/` | result: pass | note: 强制 PKCE + 删 pkce_required（迁移 0008）后全包绿；oauth/e2e/httpapi 的 PKCE 流程测试通过（confidential 现需 secret+verifier）；integration 编译过。
- 2026-06-25 p6-3 | stack: ui | command: `pnpm build`(tsc -b && vite build) + `pnpm lint` | result: pass | note: 删 Require PKCE 勾选框、`pkce_required` 字段后打包绿（JS 463KB/gzip 145KB），lint 0 error。

- 2026-06-25 p6-4 | stack: go | command: `go build ./...` + `go test ./internal/httpapi/` | result: pass | note: regenerate-secret 端点测试绿（无 step-up→401/403、新 secret≠原、未知 client→404）；httpapi 全包绿。
- 2026-06-25 p6-4 | stack: ui | command: `pnpm build`(tsc -b && vite build) + `pnpm lint` | result: pass | note: Copy ID + Regenerate secret（step-up + 一次性 secret 面板）打包绿（JS 465KB/gzip 146KB），lint 0 error。

- 2026-06-26 p6-5 | stack: go | command: `go test ./internal/core/oauth/ ./internal/core/keys/` + `make -n dev-seed-client` + openssl/Go challenge 比对 | result: pass | note: 新负向测试 `TestToken_AuthCodeConfidential`（无 verifier→invalid_grant）绿；oauth/keys 全包绿；Makefile recipe 解析正常、INSERT 仅现存列；challenge `usxdYisxoKb8I2kM8-BRMVmatPuo8ZkJVF8eyrFQsk8` openssl==Go。
...
- 2026-06-26 多智能体评审（Claude+Cursor）发现处置（用户拍板"只修值得现在修的"）：**已修（p6-5）**=Makefile dev-seed、confidential 无 verifier 负向测试、两处注释。**Defer（有外部接入方或做测试加固时再补）**=① `code_challenge_method`/格式校验（安全风险为零，S256 已实质强制；仅 RFC 完整性+报错可读性）；② retire 端点 HTTP 层测试、public 客户端 secret 再生 409 测试、authorize 缺 challenge 负向测试（底层逻辑已测，属覆盖补全）。**Wontfix（成本>收益）**=③ `client_id` 碰撞检测（72-bit，概率 ~10⁻²¹）；④ admin GET `no-store` 全局（有意设计，非仅 JWKS）；⑤ `keys.Retire` TOCTOU（owner+step-up 单操作，最坏重复 SetStatus）；⑥ `CopyButton` 无 clipboard 仍提示（非安全上下文边角）。评审产物存 `code_review/S0002-keys-oauth-hardening/`（git-ignored）。
