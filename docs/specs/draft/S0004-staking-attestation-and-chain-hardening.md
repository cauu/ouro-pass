...
- **CachedSource(epoch 口径缓存)**:接入 `StakeSnapshotCache`;回源/退休/single-flight/负缓存;reconciler 兼任 epoch 边界刷新 + state 重算。
...
- C2 **缓存 = epoch 口径**:established(active)只信 `account_stake_history`;缓存有效 iff `cached.epoch == 当前 epoch`;回源仅 `miss | epoch 滚动`;不引入中途时间 TTL(pending 子集除外,可选)。
...
- D4 **缓存 epoch 口径**:valid iff `cached.epoch==当前`;回源 `miss | rollover`;single-flight + 超时;负缓存按 epoch;不活跃 credential 退休;reconciler 兼刷。
...
- D9 **Koios 失败策略**:默认 fail-closed(拒发);可配"Koios 不可达时放行上一 epoch 旧值 + 告警"。
...
### 2.3 CachedSource(包住真 Source)
- 接口不变(`Snapshot(sch)`),`evaluate()` 不改。
- 读:命中且 `cached.epoch==当前` → 返回;否则视为 miss → 回源。
- 回源:打 Koios(account_info,必要时 account_stake_history)→ 写 `StakeSnapshotCache`(epoch + fetched_at)→ **single-flight**(同 sch 并发合一)+ 超时 + 失败按 D9。
- 负缓存:不合格也按 epoch 缓(口径 A 下整 epoch 正确)。
- 刷新/退休:**reconciler** epoch 边界刷活跃集合 + 重算 state;不活跃 credential(无活跃订阅 + LRU)驱逐,封顶规模与 Koios 负载。
...
- [ ] p2-2 `CachedSource`:epoch 口径缓存 + 回源(miss|rollover)+ single-flight + 超时 + 负缓存 + D9 失败策略;接入 `StakeSnapshotCache`。
...
- TC-3 CachedSource:命中(同 epoch)、回源(miss/rollover)、single-flight、负缓存、退休、D9 失败策略。
...
- 2026-06-25 核心决策(用户拍板):① issuer 定位 = 质押身份证明提供方,业务策略下沉 RP;② 第一方渠道保留薄 tier 映射(option a);③ rules 的"阈值→tier 策略"删除/下沉,保留 pool 闸 + 三态/facts 提取 + token claims;④ 有效质押 = epoch active_stake 口径,pending 仅入场过渡,leaving 由 epoch 自然收敛、grace 下沉 RP;⑤ 缓存 epoch 口径(miss|rollover 回源 + 退休 + single-flight + 负缓存);⑥ 附带 admin owner 链上校验、operator/viewer 管理、delegator 枚举三项后端加固(可独立排期)。
