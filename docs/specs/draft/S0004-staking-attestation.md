...
- D9 **delegator 枚举(C)**:`chain.Delegators` via Koios `/pool_delegators` 或 db-sync;与 A 解耦,可延后。
...
- `chain.Source` 加 `Delegators(ctx, poolID) ([]string, error)`(Koios `/pool_delegators` 分页 → `StakeHashFromRewardAddress` 转 hash;db-sync 可选;node_lsq 返回 not-implemented)。
...
## 3. Execution Plan
- [ ] p1-1 Koios 升级:`account_info` + `account_stake_history`,`Snapshot` 扩展(live/active pool、真 active_stake、epochs_active、status);单测(滞后向量)。
...
- [ ] p5-1 (可选/可延后) delegator 枚举:`chain.Delegators` + `GET /api/admin/delegators` + 测试。
