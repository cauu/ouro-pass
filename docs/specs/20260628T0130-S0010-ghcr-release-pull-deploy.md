...
### p4 文档
- [x] p4-1 `docs/deployment.md` 改拉取式：最小文件清单 + `docker run stake-hash` + 升级 + 本地构建备选 + GHCR 包公开提示（TC-3, TC-6）。

### p5 验收
- [x] p5-1 验收：子命令与 `stakehash` 输出一致、`go build/vet` + `make test` 绿、`docker compose config` 用新 image 通过、`release.yml` lint 通过、Dockerfile 多架构构建可用（TC-1..TC-6 汇总）。
...
- TC-6 | stack: other | command: review docs/deployment.md | result: pass | note: 最小文件清单 + curl 下载 + docker run stake-hash + pull 升级 + 本地构建备选 + GHCR 公开提示，与拉取式 compose/子命令一致
- TC-1..TC-6 | stack: go/docker/shell | command: p5-1 汇总 | result: pass | note: go build/vet 绿、子命令一致、buildx amd64+arm64 OK、actionlint OK、compose config OK、交付物齐备、无新依赖；真实 tag→CI→GHCR 推送留待维护者(Exception #3)
