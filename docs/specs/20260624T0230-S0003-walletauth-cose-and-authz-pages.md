...
- 2026-06-24 手测进展（用户）：Vespr 在 `/bind` 实测——修复前 `access_denied`（暴露 p3-1-fix1）；修复后用户复测返回 **`not_eligible`(403)** = ✅ 签名路径全通（COSE_Key 抽 vkey + COSE_Sign1 验签 + hash 绑定全过；仅 mock 链无该凭证质押数据故不合格，符合预期）。真钱包矩阵已过 1 个（Vespr）；fix 为通用（容忍任意混合键 header）+ 已锁 golden vector。第 2 个钱包为可选增信。
