...
涉及接口：`POST /api/keys/issuer/generate`、`POST /api/keys/issuer/rotate`、`POST /api/oauth/revoke`、`GET /.well-known/poolops/jwks.json`；敏感操作需 admin step-up。
...
| GET | `/api/admin/members[/:sub]` | Admin | viewer+ | 派生名册 / 详情 |
