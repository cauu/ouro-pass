...
| 规则与身份 | MembershipRule · StakeSnapshotCache(可选) ; 身份键用 stake_credential_hash，无 Member 表 | ✅/○ |
...
| 字段 | 类型 | 约束 | 说明 |
|---|---|---|---|
| kid | text | PK | 如 `pao-issuer-2026-08` |
...
- **手动拉黑**：真需要时再建稀疏 `Blacklist(stake_credential_hash, reason, created_at)`，不为它养整张表。
...
|---|---|---|---|
| nonce | text | PK | |
| purpose | enum | | `member_auth`/`admin_login`/`step_up` |
...
| GET | `/.well-known/poolops/jwks.json` | Verifier | 无 | 签名公钥（无证书链） |
...
| GET | `/api/admin/members` | Admin | viewer+ | 派生名册 |
| GET | `/api/admin/members/:sub` | Admin | viewer+ | 会员详情 |
| POST | `/api/admin/members/:sub/revoke` | Admin | operator+ | 拉黑 |
...
**验证** — 消费方从 `GET /.well-known/poolops/jwks.json` 取 `kid` 对应公钥验签（一组签名公钥，无证书链），或调 `/api/oauth/introspect` 在线校验。
...
`GET /.well-known/poolops/jwks.json` — 标准 JWKS，仅签名公钥，**无证书链**：

```json
{ "keys":[ { "kid":"pao-issuer-2026-08", "kty":"OKP", "crv":"Ed25519", "x":"<pub>", "status":"active" } ] }
```
...
| 端点 | 方法 | 角色 | 说明 |
|---|---|---|---|
| `/api/admin/members` | GET | viewer+ | 派生名册（聚合 active session/token，按 sub） |
| `/api/admin/members/:sub` | GET | viewer+ | 单会员详情（现算资格 + 其 session/token） |
| `/api/admin/members/:sub/revoke` | POST | operator+ | 拉黑（写 Blacklist + 撤销其 token/grant/session） |
